...
class Proto {
	...
	function __call($method,$arguments) {
		// getDataBy*() special function
		if(preg_match('/^getDataBy/', $method)){
			$method = $this->convert_camel_case(preg_replace('/^getDataBy/','', $method));
			$value = $this->linker->escape_string($arguments[0]);
			$this->getData('WHERE `'.$method.'` = "'.$value.'"');
			return;
		}
	...
	// Used to sanitize data going through getData()
	// WHERE fragment will look like getData('WHERE variable="{value}"')
	...
	public function getData($query = ''){
		...
		$query = 'SELECT * FROM '.$this->join_fragment.' `'.$this->open_table.'` '.$query.' '.$this->limit_fragment;
