...
class XTemplate {
	...
	public function restart ($file, $tpldir = '', $files = null, $mainblock = 'main', $autosetup = true, $tag_start = '{', $tag_end = '}') {
		...
		// Modified by Sam Peterson May 25 2014 - Because fuck removing data
	...
	public function assign_file ($name, $val = '') {
      ...
      * 
      * @access public
      * @param string $bname Block name to check existence of
      * 
      * Added by Sam Peterson May 25 2014 - because fuck static shit
      */
       public function check ($bname) {
	       return array_key_exists($bname,$this->blocks);
       }
		...
		foreach($con2 as $k => $v) {
			...
			if (preg_match_all("/$patt/ims", $v, $res, PREG_SET_ORDER)) {
				...
				if (strtoupper($block_word) == $this->block_start_word) {
					...
					// Added by Sam Peterson May 17 2014 - Because fuck blank lines
					$blocks[$cur_block_name] = preg_replace("/(^[\r\n]*|[\r\n]+)[\s\t][\r\n]+/", "", $blocks[$cur_block_name]);
					// End add by Sam Peterson 
